...
pub struct Job<T> {
    clos_fold:
        Box<dyn serde_traitobject::FnOnce<(Rc<AtwThreadWorker>, String), Output = ()> + 'static>,
    _phantom: PhantomData<T>,
}

impl<T> Job<T>
where
    T: Future<Output = ResultJJ> + 'static,
{
    pub fn from_clos<F>(clos: F) -> ArrayBuffer
    where
        F: MtClosure,
    {
        let vec: Vec<u8> = bincode::serialize(&clos).unwrap();
        {
            ...
            {
                (Self {
                    clos_fold: Box::new(FnOnce!(
                        move |atw_thw: Rc<AtwThreadWorker>, req_id: String| {
    ...
    pub fn from_aclos<F>(clos: F) -> ArrayBuffer
    where
        F: MtAsyncClosure<T>,
    {
        let vec: Vec<u8> = bincode::serialize(&clos).unwrap();
...
mod tests {
    ...
    async fn closures() {
        ...
        assert_eq!(exec_fn(FnOnce!(move || Ok(JsValue::from(42)))), ok42);
        assert_eq!(
            exec_aclos(FnOnce!(async move || Ok(JsValue::from(42)))).await,
            ok42
        );
        assert_eq!(exec_pin(FnOnce!(move || Box::pin(afn_foo(42)))).await, ok42);
        assert_eq!(
            exec_pin(FnOnce!(move || Box::pin((async move || Ok(
                JsValue::from(42)
            ))())))
            .await,
            ok42
    ...
    async fn afn_foo(s: u32) -> ResultJJ {
        Ok(JsValue::from(s))
    }

    fn exec_fn<F>(clos: F) -> ResultJJ
    where
        F: FnOnce() -> ResultJJ + Serialize + DeserializeOwned,
    {
    ...
    async fn exec_aclos<F, G>(clos: F) -> ResultJJ
    where
        F: FnOnce() -> G + Serialize + DeserializeOwned,
        G: Future<Output = ResultJJ>,
    {
        let vec = bincode::serialize(&clos).unwrap();
        let recovered: F = bincode::deserialize(&vec).unwrap();
        recovered().await
    ...
    async fn exec_pin<F, G>(clos: F) -> ResultJJ
    where
        F: FnOnce() -> Pin<Box<G>> + Serialize + DeserializeOwned,
        G: Future<Output = ResultJJ>,
    {
        let vec = bincode::serialize(&clos).unwrap();
        let recovered: F = bincode::deserialize(&vec).unwrap();
